...
const MongoStore = require('connect-mongo'); //to store session informations on the cloud server mongoatlas
...
const dbURL = process.env.ATLASDB_URL; //db on mongostlas
...
const store = MongoStore.create({ //to save the session information on mongoatlas rather than local memeory
    mongoUrl:dbURL,
    crypto:{
        secret:process.env.SECRET
    },
    touchAfter: 24*3600, //time(inseconds) after which sesiion refreshes/updates if no action is perfromed {i.e logged out}
})
